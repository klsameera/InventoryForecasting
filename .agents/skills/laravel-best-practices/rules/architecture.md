...
> This application uses a strict **Facade + Service + thin Controller**
> architecture. The "Single-Purpose Action Classes" and "Use Dependency
> Injection (into controllers)" sections below do **not** apply here:
> - No Action classes. Business logic lives in `domain/Services/{Module}Service/`.
> - Controllers never inject a service. They call `domain/Facades/{Module}Facade/`.
> - No repository pattern.
>
> See `.ai/rules/architecture.md` and the `module-generation` skill. Everything
> else on this page (default sort, atomic locks, mb_* helpers, defer(), Context,
> Concurrency, convention over configuration) still applies.
